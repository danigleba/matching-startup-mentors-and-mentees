const getAllComments = async (videoId, apiKey, pageToken = "") => {
  ...
  if (nextPageToken) {
    const nextPageComments = await getAllComments(videoId, apiKey, nextPageToken)
...
export default async function handler(req, res) {
  ...
  try {
    const allComments = await getAllComments(videoId, apiKey)
